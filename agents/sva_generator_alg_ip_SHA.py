...
def _rel_fwd(from_dir: str, to_path: str) -> str:
    rp = os.path.relpath(os.path.abspath(to_path), os.path.abspath(from_dir))
    return rp.replace("\\", "/")
...
_CLK_ALIASES = ["clk_i", "clk", "clock", "i_clk", "aclk", "core_clk"]
_RST_ALIASES = ["rst_i", "rst", "reset", "reset_i", "rst_n", "reset_n", "aresetn", "rstni", "resetb", "rstb", "nreset"]
...
def generate_sva_for_sha_property(
    *, category: str, property_name: str, design_path: str, analysis_path: str
    ...
    if not dut:
        raise RuntimeError(
            "SHA roles file must contain role 'dut' with chosen module name.\n"
            f"Found roles: {sorted(list(roles.keys()))}\n"
            f"analysis_path={analysis_path}"
        )

    text_out = roles.get("text_out")
    if not text_out:
        raise RuntimeError(
            "SHA roles file must contain role 'text_out' with chosen signal name (e.g., text_o).\n"
            f"Found roles: {sorted(list(roles.keys()))}\n"
            f"analysis_path={analysis_path}"
        )
    ...
    sva_prop_id = property_name if property_name.startswith("p_") else f"p_{property_name}"
    ...
    sva = f"""// SHA outputs differ (detect change over time)
property {sva_prop_id};
   @(posedge {u}.{clk_port}) disable iff ({rst_expr})
     ##[1:$] ({u}.{text_out} != $past({u}.{text_out}, 1));
endproperty
_assert_1: assert property ({sva_prop_id});"""
    ...
    conns: List[str] = [f"    .{clk_port}({clk_port})"]
    if rst_port:
    ...
    design_include = _rel_fwd(GENERATED_DIR, design_path)
    ...
    logger.info("SHA algo wrapper: %s", os.path.abspath(sv_out))
    logger.info("SHA algo tcl    : %s", os.path.abspath(tcl_out))
    logger.info(
        "DUT=%s | text_out=%s | clk_port=%s | rst_port=%s (active_low=%s) | roles_file=%s",
        dut, text_out, clk_port, (rst_port or "<none>"), active_low, analysis_path
    )

    if rst_port and active_low:
        logger.warning(
            "Reset port '%s' looks active-low. Property uses disable iff(!u_dut.%s). "
            "If your Jasper reset command needs explicit polarity, adjust the TCL accordingly.",
            rst_port, rst_port
        )
