...
class PDGBuilder:
    def __init__(self, module_node=None):
        ...
        self._mod_lhs_candidates: Set[str] = set()
        self.message_vars: List[str] = []

        if module_node:
            self.build_pdg(module_node)

    def _extract_any_var_name(self, expr: Any) -> Optional[str]:
        ...
        if kind in ('ElementSelect', 'RangeSelect'):
            return self._extract_any_var_name(expr.get('value', {}))
        if kind == 'MemberAccess':
            base = self._extract_any_var_name(expr.get('parent', {}))
            member = expr.get('member')
            if base and member:
                return f"{base}.{member}"
            return base
        if kind == 'Conversion':
            return self._extract_any_var_name(expr.get('operand', {}))
        if kind == 'UnaryOp':
            return self._extract_any_var_name(expr.get('operand', {}))
        if kind == 'BinaryOp':
            return self._extract_any_var_name(expr.get('left', {}))
        if kind == 'ConditionalOp':
            v = self._extract_any_var_name(expr.get('left', {}))
            if v:
                return v
            return self._extract_any_var_name(expr.get('right', {}))
        return None

    def _scan_for_mod_ops(self, node: Any) -> None:
        if isinstance(node, dict):
            if node.get('kind') == 'BinaryOp':
                op = node.get('op') or node.get('operator') or ''
                op_l = str(op).strip().lower()
                if op_l in ('mod', 'modulo', '%'):
                    lhs = self._extract_any_var_name(node.get('left', {}))
                    if lhs:
                        self._mod_lhs_candidates.add(lhs)
            for v in node.values():
                self._scan_for_mod_ops(v)
        elif isinstance(node, list):
            for v in node:
                self._scan_for_mod_ops(v)

    def _finalize_message_role(self) -> None:
        inputs = set([str(x) for x in self.input_list])
        chosen = set(self._mod_lhs_candidates) & inputs
        if not chosen:
            chosen = set(self._mod_lhs_candidates)
        self.message_vars = sorted(chosen)
    ...
    def extract_sources(self, expression):
        ...
        if kind == 'NamedValue':
            ...
            cond = expression.get('cond')
            if cond:
                cond_sources = self.extract_sources(cond)
            else:
                cond_sources = {'names': [], 'ops': []}
                for c in expression.get('conditions', []) or []:
                    if isinstance(c, dict) and 'expr' in c:
                        cs = self.extract_sources(c.get('expr', {}))
                        cond_sources['names'].extend(cs['names'])
                        cond_sources['ops'].extend(cs['ops'])
        ...
        elif kind in ('IntegerLiteral', 'UnbasedUnsizedIntegerLiteral', 'BasedIntegerLiteral'):
            pass
    ...
    def extract_condition_str(self, expr):
        ...
        if kind == 'NamedValue':
            ...
            left = self.extract_condition_str(expr.get('left', {}))
            right = self.extract_condition_str(expr.get('right', {}))
            op = expr.get('op', '')
            return f'({left} {op} {right})'
        elif kind == 'UnaryOp':
            operand = self.extract_condition_str(expr.get('operand', {}))
            op = expr.get('op', '')
            return f'({op}{operand})'
        elif kind in ('IntegerLiteral', 'UnbasedUnsizedIntegerLiteral', 'BasedIntegerLiteral'):
            return expr.get('value', '') or expr.get('constant', '') or ''
        else:
            return ''
    ...
    def build_pdg(self, module_node):
        self._scan_for_mod_ops(module_node)
...
def export_pdg_pack_json(*,
    ...
    message_vars = set(getattr(builder, 'message_vars', []) or [])
    mod_lhs_candidates = sorted(list(getattr(builder, '_mod_lhs_candidates', set()) or set()))
    ...
    for name, node in builder.pdg.items():
        ...
        if name in message_vars:
            nodes[name]["role"] = "message"
    ...
    role_by_var = {}
    if message_vars:
        roles["message"] = sorted(message_vars)
        for v in roles["message"]:
            role_by_var[v] = "message"
    ...
    pack = {
        ...
        "roles": roles,
        "role_by_var": role_by_var,
        "mod_lhs_candidates": mod_lhs_candidates,
    }
